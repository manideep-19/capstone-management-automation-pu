...
export const SignupForm: React.FC = () => {
  ...
  const validateStudentEmail = (email: string): boolean => {
    ...
    // Pattern: firstname.rollnumber@presidencyuniversity.in (case insensitive)
    const studentEmailPattern = /^[a-z]+\.\d{11}@presidencyuniversity\.in$/;
  ...
  const extractRollNumberFromEmail = (email: string): string => {
    ...
    const lowerEmail = email.toLowerCase();
    const match = lowerEmail.match(/^[a-z]+\.(\d{11})@presidencyuniversity\.in$/);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.role === 'student') {
      ...
      if (formData.rollNo.length !== 11 || !/^\d{11}$/.test(formData.rollNo)) {
